...
class Conv(object):
    ...
    def backward(dout, cache):
        ...
        dx_pad = torch.zeros_like(x_pad, device=dout.device, dtype=dout.dtype)
                ...
                for k in range(H_out):
                    for m in range(W_out):
                        idx = x[
                            ...
                            m * stride : m * stride + pool_width,
                        ].argmax()
                        dx[
                            i,
                            j,
                            k * stride + idx // pool_height,
                            m * stride + idx % pool_height,
                        ] += dout[i, j, k, m]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        H_out = H // 2
        W_out = W // 2
        D = num_filters * H_out * W_out
        self.params["W2"] = (
            torch.randn(D, hidden_dim, dtype=dtype, device=device) * weight_scale
        )
...
class DeepConvNet(object):
    ...
    def __init__(
        ...
        for i in range(len(num_filters)):
            self.params[f"W{i+1}"] = (
                kaiming_initializer(
                    last_channel, num_filters[i], 3, device=device, dtype=dtype
                )
                if weight_scale == "kaiming"
                else torch.randn(
                    num_filters[i], last_channel, 3, 3, dtype=dtype, device=device
                )
                * weight_scale
            )
        ...
        self.params[f"W{self.num_layers}"] = (
            kaiming_initializer(D, num_classes, device=device, dtype=dtype, relu=False)
            if weight_scale == "kaiming"
            else torch.randn(D, num_classes, dtype=dtype, device=device) * weight_scale
        )
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            if i in self.max_pools:
                if self.batchnorm:
                    out, conv_cache = Conv_BatchNorm_ReLU_Pool.forward(
                        out,
                        self.params[f"W{i+1}"],
                        self.params[f"b{i+1}"],
                    ...
                    out, conv_cache = Conv_ReLU_Pool.forward(
                        out,
                        self.params[f"W{i+1}"],
                        self.params[f"b{i+1}"],
                        conv_param,
                ...
                if self.batchnorm:
                    out, conv_cache = Conv_BatchNorm_ReLU.forward(
                        out,
                        self.params[f"W{i+1}"],
                        self.params[f"b{i+1}"],
                        self.params[f"gamma{i+1}"],
                    ...
                    out, conv_cache = Conv_ReLU.forward(
            ...
            if i in self.max_pools:
                if self.batchnorm:
                    (
                        ...
                        grads[f"gamma{i+1}"],
                        grads[f"beta{i+1}"],
                    ) = Conv_BatchNorm_ReLU_Pool.backward(dout, conv_caches[i])
                else:
                    (
                        dout,
                        grads[f"W{i+1}"],
                        grads[f"b{i+1}"],
                    ) = Conv_ReLU_Pool.backward(dout, conv_caches[i])
            else:
                if self.batchnorm:
                    (
                        dout,
                        ...
                        grads[f"gamma{i+1}"],
                        grads[f"beta{i+1}"],
                    ) = Conv_BatchNorm_ReLU.backward(dout, conv_caches[i])
                else:
                    (dout, grads[f"W{i+1}"], grads[f"b{i+1}"]) = Conv_ReLU.backward(
                        dout, conv_caches[i]
                    )
...
class BatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        dx_hat = dout * gamma
        dsample_var = torch.sum(
            dx_hat * (x - sample_mean) * (-1.0) / 2 * (sample_var + eps) ** (-3.0 / 2),
            dim=0,
        )

        dx1 = (
            dx_hat / torch.sqrt(sample_var + eps)
            + dsample_var * 2 * (x - sample_mean) / N
        )
        dsample_mean = torch.sum(-dx1, dim=0)
        dx = dx1 + dsample_mean / N
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = dout.shape
        dx, dgamma, dbeta = BatchNorm.backward_alt(
